...
output "replicated_vms_info" {
  ...
  value = [for vm_name, vm in azurerm_site_recovery_replicated_vm.replicated_vm : {
    vm_name                              = vm_name
    replicated_vm_id                     = vm.id
    target_capacity_reservation_group_id = vm.target_capacity_reservation_group_id
  }]
}
