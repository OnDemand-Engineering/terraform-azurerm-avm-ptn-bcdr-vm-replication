...
resource "azurerm_site_recovery_replicated_vm" "replicated_vm" {
  ...
  dynamic "managed_disk" {
    ...
    content {
      ...
      staging_storage_account_id    = var.storage_account_creation_enabled ? azurerm_storage_account.staging.id : provider::azapi::build_resource_id("/subscriptions/${data.azapi_client_config.current.subscription_id}", "Microsoft.Storage/storageAccounts", var.storage_account_name)
      target_resource_group_id      = each.value.target_resource_group_id
